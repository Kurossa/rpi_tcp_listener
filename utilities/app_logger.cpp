...
/* Log level */
static char s_acLevel[LOG_MAX_LEVEL][LOG_MAX_LEVEL_NAME] = { "INF", "WAR",
		"ERR", "INF" };
...
static void logGetCompactTimestamp(char *pStrBuf) {
	...
	sprintf(pStrBuf, "%d%02d%02d%02d%02d%02d", tm_now->tm_year + 1900,
...
static int logChange() {
	...
	sprintf(acFileNameNew, "%s%s_%s_%02d.txt", pLogDirName_s, pLogFileName_s,
			acDate, LogFileNum_s);
...
static void logRemoveLogs(void) {
	...
	for (int i = 0; i < ((int) logFileNames.size() - MAX_LOG_FILES + 1);
			++i, ++it) {
		printf("LOG FILE: %s\n", it->c_str());
		remove(it->c_str());
	}
...
int logInit() {
	...
	if (!stat(pLogDirName_s, &sb) == 0 || !S_ISDIR(sb.st_mode)) {
		int status = mkdir(pLogDirName_s,
...
void logPrintf(int iLevel, const char* pStrFormat, ...) {
	...
	if (iLevel == ERROR_LOG || iLevel == SCREEN_LOG) {
		printf("%s", acBuf);
	}
